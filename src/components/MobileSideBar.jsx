import React from "react";
import home from "../assets/sidebar-images/home-side.svg";
import explore from "../assets/sidebar-images/explore-side.svg";
import subcription from "../assets/sidebar-images/subscriptions-side.svg";
import youtubeMusic from "../assets/sidebar-images/youtube-music-side.svg";
import library from "../assets/sidebar-images/library-side.svg";
const MobileSideBar = () => {
  return  (
    <div className="w-40 z-50 bg-gray-200 h-[100vh]">
      <div className="px-0 align-middle ">
        <div className="flex items-center my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img src={home} alt="home" className="h-8" />
          <span className="font-medium text-base ml-4">
            Home
          </span>
        </div>
        <div className="flex items-center my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img
            src="data:image/png;base64,"
            alt="Shorts"
            className="h-8"
          />
          <span className="ml-4 text-base ">Shorts</span>
        </div>
        <div className="flex items-center  my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img src={library} alt="library" className="h-8" />
          <span className="ml-4 text-base ">Library</span>
        </div>
        <div className="flex items-center  my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img src={explore} alt="explore" className="h-8" />
          <span className="ml-4 text-base ">Explore</span>
        </div>
        <div className="flex items-center  my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img
            src={subcription}
            alt="subcription"
            className="h-8"
          />
          <span className="ml-4 text-base">Subcription</span>
        </div>
        <div className="flex items-center  my-1 py-1 hover:bg-[#E0DDDD] cursor-pointer transition">
          <img
            src={youtubeMusic}
            alt="youtubeMusic"
            className="h-8"
          />
          <span className="ml-4 text-base">Music</span>
        </div>
      </div>
    </div>
  );
};

export default MobileSideBar;
